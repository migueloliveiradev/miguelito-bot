...
{
    internal class Poke : ApplicationCommandModule
    {
        [SlashCommand("Poke", "Reactions ┇ Send a Gif of poke")]
        public async Task PokeUser(InteractionContext ctx, [Option("user", "member you want to poke")] DiscordUser user)
        {
            string url = "https://api.waifu.pics/sfw/poke";
            string json = await new HttpClient().GetStringAsync(url);
            dynamic? data = JsonConvert.DeserializeObject(json);
            string? gif = data?.url;
            DiscordEmbedBuilder embed = new()
            {
                Description = $"{ctx.Member.Mention} cutucou {user.Mention}",
            ...
            await Methods.CommandsUsed("Reactions Poke", ctx.Guild.Id, ctx.User.Id);
            ctx.Client.ComponentInteractionCreated += async (s, e) =>
            {
                if (e.User == user && ctx.GetOriginalResponseAsync().Result.Id == e.Message.Id)
                {
                    ...
                    if (e.Id == "GiveBack")
                    {
                        ...
                        DiscordEmbedBuilder embed = new()
                        {
                            Description = $"{user.Mention} devolveu a cutucada de {ctx.Member.Mention}",
                            Color = Variables.Cores(),
                            ImageUrl = gif,
                        };

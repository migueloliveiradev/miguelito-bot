            ...
            [Option("Reason", "User Kick reason")] string reason = "NULL")
        {
            if (user == ctx.Guild.Owner)
            {
                await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                    new DiscordInteractionResponseBuilder().WithContent($"Você não pode expulsar o dono.").AsEphemeral(true));
                return;
            }
            else if (user == ctx.User)
            {
                await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                    new DiscordInteractionResponseBuilder().WithContent($"Você não pode se expulsar.").AsEphemeral(true));
                return;
            }
            else if (ctx.Guild.GetMemberAsync(user.Id).Result.Hierarchy >= ctx.Member.Hierarchy)
            {
                await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                    new DiscordInteractionResponseBuilder().WithContent($"Você não expulsar esse membro pois ele tem um cargo maior ou equivalente que o seu.").AsEphemeral(true));
                return;
            }
            ...
            {
                await ctx.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource,
                    new DiscordInteractionResponseBuilder().WithContent("O motivo da expulsão deve ser menor que 450 caracteres").AsEphemeral(true));
                return;
            }
